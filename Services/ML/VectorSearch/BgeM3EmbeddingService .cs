...
namespace Sammlerplattform.Services.ML.VectorSearch
{
    public class M3Embedder : IDisposable
    {
        ...
        /// <param name="trackEvents">Service for tracking events</param>
        public M3Embedder(string tokenizerPath, string modelPath, M3EmbedderConfig config)
        {
            _config = config;

            // 1. Tokenizer Session (erwartet "inputs" als string)
            var tokenizerOptions = CreateSessionOptions(forTokenizer: true);
            tokenizerOptions.RegisterOrtExtensions();
            _tokenizerSession = new InferenceSession(tokenizerPath, tokenizerOptions);  // ← Das ist eigentlich der Tokenizer

            // 2. Model Session (erwartet "input_ids", "attention_mask")
            var modelOptions = CreateSessionOptions(forTokenizer: false);
            modelOptions.RegisterOrtExtensions();
        ...
        /// </summary>
        private Microsoft.ML.OnnxRuntime.SessionOptions CreateSessionOptions(bool forTokenizer)
        {
